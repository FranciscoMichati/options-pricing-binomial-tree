...
class BinomialOptionTree():
    ...
    def return_tree(self, plot = True, text_trees = True):
        ...
        if self.mode == 'lite':
            ...
            if text_trees:
                ...
                with open('./output/Asset_prices_tree.txt', 'w') as f:
                    root = build(asset_prices_flat_values)
                    print(root, file=f)
                with open('./output/Options_tree.txt', 'w') as f:
            ...
            if plot:
                ...
                BinomialOptionTree.plot_binary_tree(asset_prices_flat_values, title='Asset prices tree')
                BinomialOptionTree.plot_binary_tree(options_prices_flat_values, title='Option prices tree')
            



    @staticmethod
    def plot_binary_tree(data: list, title: str, node_size: int = 500, font_size: int = 5):
    ...
    @staticmethod
    def hierarchy_pos_aux(G, root, width: float=1., vert_gap: float=0.2, vert_loc:float=0, xcenter: float=0.5, 
        ...
        if len(children) != 0:
            ...
            for child in children:
                nextx += dx
                pos = BinomialOptionTree.hierarchy_pos_aux(G, child, width=dx, vert_gap=vert_gap, vert_loc=vert_loc-vert_gap, xcenter=nextx, pos=pos, parent=root, parsed=parsed)
        return pos

    @staticmethod
    def hierarchy_pos(G, root, width: float=1., vert_gap: float=0.2, vert_loc: float=0, xcenter: float=0.5):
        pos = BinomialOptionTree.hierarchy_pos_aux(G, root, width, vert_gap, vert_loc, xcenter)
        return pos
